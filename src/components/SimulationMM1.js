...
import QueueingCalculator from "./Simulation/MM1/QueueingCalculator";

const SimulationMM1 = () => {
  const [activeTab, setActiveTab] = useState("random");
  const [randomData, setRandomData] = useState([]);
  const [calculatedData, setCalculatedData] = useState([]);
  const [ QueueingCalulate, setQueueingCalculate] = useState([]);
  ...
  useEffect(() => {
    ...
    const InterarrivalMeanParam = parseFloat(params.get("InterArrivalMean"));
    const serviceMeanParam = parseFloat(params.get("serviceMean"));
    console.log(InterarrivalMeanParam)
    ...
    const M = parseFloat(params.get("Mvalue"));
      ...
      setInterArrivalMean(InterarrivalMeanParam)
      ...
      const queuingdata = queueingData(serviceMean,InterArrivalMean);
      setQueueingCalculate(queuingdata);
  ...
  const queueingData =(serviceMean,InterArrivalMean) => {
   
    const queuingdataVars = []
    console.log("queueing data :")
    console.log(InterArrivalMean)
    console.log(serviceMean)
    const WQ = InterArrivalMean / (serviceMean * (serviceMean - InterArrivalMean));
    const WS = 1 / (serviceMean - InterArrivalMean);
    const LS = InterArrivalMean / (serviceMean - InterArrivalMean);
    const LQ = (InterArrivalMean * InterArrivalMean) / (serviceMean * (serviceMean - InterArrivalMean));

    queuingdataVars.push(WQ, WS, LQ, LS);

    return queuingdataVars;
  }
  ...
  const calculateCalculatedData = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (!stop_loop) {
        ...
        if (skipped_customers.length !== 0 && task.priority >= skipped_customers[0].priority) {
       ...
       else if (stopped_customers.length !== 0 && task.priority >= stopped_customers[0].priority) {
        let remaining_time;
        let turnaroundTime;
        let util_time;
        while (stopped_customers.length !== 0) {
            ...
            if (task.priority >= currentCustomer.priority) {
                ...
                const waitTime = currentCustomer.waitTime;
                 ...
                 turnaroundTime = currentCustomer.endTime - currentCustomer.arrivalTime;
  ...
  return (
    ...
    >
      <div className="w-full max-w-3xl flex justify-center space-x-4 my-8">
        {/* <button
          className={`tab-button bg-gray-50 border styled text-lg rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 ${
        ...
        <button
          ...
          onClick={() => handleTabChange("queueing")}
        >
          Queueing Calculator
        </button>
      </div>
      ...
      <div className="w-full ">
        ...
        {activeTab === "queueing" && (
          <QueueingCalculator QueueingCalulate={QueueingCalulate} />
        )}
      </div>
    </div>
  );
